package com.vn.hcmute.team.cortana.mymoney.usecase.base;

/**
 * Created by infamouSs on 8/11/17.
 */

public class Action {
    
    public static final String ACTION_LOGIN_NORMAL = "action_login_normal";
    
    public static final String ACTION_REGISTER = "action_register";
    
    public static final String ACTION_GET_IMAGE = "action_get_image";
    
    
    public static final String ACTION_CREATE_WALLET = "create_wallet";
    public static final String ACTION_UPDATE_WALLET = "update_wallet";
    public static final String ACTION_DELETE_WALLET = "delete_wallet";
    public static final String ACTION_GET_WALLET = "get_wallet";
    public static final String ACCTION_MOVE_WALLET = "move_wallet";
    
    public static final String ACCTION_GET_CURRENCIES = "get_currencies";
    
    public static final String ACTION_GET_EVENT = "get_event";
    public static final String ACTION_CREATAE_EVENT = "create_event";
    public static final String ACTION_UPDATE_EVENT = "update_event";
    public static final String ACTION_DELETE_EVENT = "delete_event";
    
    public static final String ACTION_GET_SAVING = "get_saving";
    public static final String ACTION_CREATE_SAVING = "create_saving";
    public static final String ACTION_UPDATE_SAVING = "update_saving";
    public static final String ACTION_DELETE_SAVING = "delete_saving";
    public static final String ACTION_TAKE_IN_SAVING = "take_in";
    public static final String ACTION_TAKE_OUT_SAVING = "take_out";
    
    
    public static final String ACTION_GET_PERSON = "get_person";
    public static final String ACTION_ADD_PERSON = "add_person";
    public static final String ACTION_REMOVE_PERSON = "remove_person";
    
    
    public static final String ACTION_GET_BUDGET = "get_budget";
    public static final String ACTION_CREATE_BUDGET = "create_budget";
    public static final String ACTION_UPDATE_BUDGET = "update_budget";
    public static final String ACTION_DELETE_BUDGET = "delete_budget";
    
    
    public static final String ACTION_GET_IMAGE_BY_ID = "action_get_image_by_id";
    
    public static final String ACTION_UPLOAD_IMAGE = "action_upload_image";
    
    public static final String ACTION_REMOVE_IMAGE = "action_remove_image";
    
    public static final String ACTION_UPDATE_IMAGE = "action_update_image";
    
}
